...
class M_saldo extends CI_Model {
        ...
        function ambilnomorbaru(){
                ...
                $yyyy  = substr($exp[0],-3);
                $mm    = $exp[1];
                $query = $this->db->query("select max(SUBSTRING_INDEX(kode_trans_pembayaran,'.',-1)*1) + 1 as new_count from t_pembayaran_header where SUBSTRING_INDEX(kode_trans_pembayaran,'.',1)='P". $yyyy. $mm ."'");
                $row   = $query->first_row();
