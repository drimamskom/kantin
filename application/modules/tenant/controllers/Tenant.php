...
class Tenant extends CI_Controller {
    ...
    public function data(){            
        ...
        if(empty($tgl_mulai)||empty($tgl_selesai)){
            ...
            $d1 = $this->m_tenant->datetosqldate($tgl_mulai);
            $d2 = $this->m_tenant->datetosqldate($tgl_selesai);
        ...
        $query1 = $this->db->query("SELECT COUNT(*) AS total, SUM(total) AS jumm 
                                    ...
                                    WHERE p.kode_trns_pemesanan IS NOT NULL $where1 AND pp.tgl='$tgl' AND stat='proses' AND dm.no_stan='$nomor_induk' ");
        ...
        $query = $this->db->query("SELECT @rownum := @rownum + 1 AS urutan, t.*
                FROM (
                        ...
                        WHERE p.kode_trns_pemesanan IS NOT NULL $where1 AND pp.tgl='$tgl' AND stat='proses' AND dm.no_stan='$nomor_induk'
                        ORDER BY p.kode_trns_pemesanan ASC $limit_txt
                ) AS t, 
        ...
        foreach ($query->result_array() as $row){
            ...
            $data[$i] = $row;
            $data[$i]['tgl']   = $this->m_tenant->sqldatetodate($row['tgl']);
            $data[$i]['button'] = '<button title="done" idnex="'.$data[$i]['id_trns_pemesanan_detail'].'" namenex="'.$data[$i]['kode_trns_pemesanan'].'" class="btn btn-primary btn-xs btndone" >Done</button>
                            <button title="batal" idnex="'.$data[$i]['id_trns_pemesanan_detail'].'" namenex="'.$data[$i]['kode_trns_pemesanan'].'" class="btn btn-danger btn-xs btnhapus" ><i class="fa fa-remove"></i> Del</button>';
            //$data[$i] = $row;  
            $i++;
        }
    ...
    public function done(){
        ...
        if($jml!='0') { 
            ...
            if($update){
                ...
                echo json_encode(array('status'=>'failed', 'txt'=>'tdk bisa konfirmasi data!'));
            }
        }else{
    ...
    public function batalpesanan(){
        ...
        // cari data masih ada         
        $query1 = $this->db->query("SELECT * FROM trns_pemesanan WHERE kode_trns_pemesanan='$name' ");
        $jml = $query1->num_rows(); 
        if($jml!='0') { 
            $row1 = $query1->first_row();
            $kode = $row1->kode;
            ...
            $query = $this->db->query("SELECT * FROM trns_pemesanan_detail WHERE id_trns_pemesanan_detail='$id' ");
            $row = $query->first_row();
            $deposit = $row->total;
            ...
            $inser2 = $this->db->query("INSERT INTO tb_deposit_moves 
                ...
                VALUES
                ('$name', '$tanggal', '$kode', '$deposit', '$datetime', '$userr') ");
            $update = $this->db->query("UPDATE trns_pemesanan_detail SET stat='$stat' WHERE id_trns_pemesanan_detail='$id' ");
