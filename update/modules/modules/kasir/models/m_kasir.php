...
class M_kasir extends CI_Model {
        ...
        function ambilnomorbaru(){
                ...
                $yyyy  = substr($exp[0],-3);
                $mm    = $exp[1];
                $query = $this->db->query("select max(SUBSTRING_INDEX(kode_trns_penjualan,'.',-1)*1) + 1 as new_count from trns_penjualan where SUBSTRING_INDEX(kode_trns_penjualan,'.',1)='P". $yyyy. $mm ."'");
                $row   = $query->first_row();
