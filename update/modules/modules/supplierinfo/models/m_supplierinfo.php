...
class M_supplierinfo extends CI_Model{	
	...
	function cek_login($table,$where){		
        ...
        function ambilnomorbaru(){
                ...
                $yyyy  = substr($exp[0],-3);
                $mm    = $exp[1];
                $query = $this->db->query("select max(SUBSTRING_INDEX(kode_faktur,'.',-1)*1) + 1 as new_count from trns_pembelian where SUBSTRING_INDEX(kode_faktur,'.',1)='F". $yyyy. $mm ."'");
                $row   = $query->first_row();
