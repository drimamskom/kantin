...
<script type="text/javascript">   
	$(function(){ 
            $('#table_cust').DataTable({
                ...
                "serverSide": true,
	        "ajax": {
	            "url": "<?php echo base_url('tenant/data'); ?>",
                    "data": function( d ) {
	    ...
	    // });  
            ...
            $(document).on( "click",".btnhapus", function() {
                    ...
                    swal({   
                        ...
                        text: "Anda yakin membatalkan pesanan : "+name+" ?",   
                        type: "warning",   
                        ...
                        function(){   
                            ...
                            $.ajax({
                                url : "<?php echo base_url('tenant/batalpesanan'); ?>",
                                type: "POST",
                                data : value,
                                dataType: "json",
                                success: function(data){
                                    if(data.status == 'success'){
                                        ...
                                        // table.ajax.reload( function ( json ) {
                                        //     $('#all_subtotal').html("Rp, "+addPeriod(json.totalz));
                                            table.ajax.url('<?php echo base_url("tenant/data"); ?>').load();
                                        // });
                                    }else{
            ...
            $(document).on( "click",".btndone", function() {
                    ...
                    swal({   
                        ...
                        text: "Apakah pesanan "+name+" sudah diantar ?",   
                        type: "warning",   
                        showCancelButton: true,   
                        //confirmButtonColor: "#DD6B55",   
                        ...
                        function(){   
                            var value = { id:id };
                            $.ajax({
                                url : "<?php echo base_url('tenant/done'); ?>",
                                type: "POST",
                                data : value,
                                dataType: "json",
                                success: function(data){
                                    if(data.status == 'success'){
                                        $.notify('Successfull order Pesanan');
                                        var table = $('#table_cust').DataTable(); 
                                        // table.ajax.reload( function ( json ) {
                                        //     $('#all_subtotal').html("Rp, "+addPeriod(json.totalz));
                                            table.ajax.url('<?php echo base_url("tenant/data"); ?>').load();
                                        ...
                                        swal("Error","Can't conform Data, error : "+name,"error");
                                    }
                                }
                            });
                        });
            ...
            $('#table_cust').DataTable().ajax.url("tenant/data").load();

	});  
    setInterval(function(){
       //$('#realtime').load("<?php // echo base_url('C_nms/realtime_all') ?>" + " #realtime");
       $('#table_cust').DataTable().ajax.url("tenant/data").load();
    ...
    <div class="col-md-12">
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Daftar Pesanan <?php echo $fullname ?></h3>
            </div>
            <div class="box-body"> 
                <!-- <form id="export_form" action="<?php echo base_url('tenant/export'); ?>" method="post" target="_self">
...
<form id="cetakform" action="<?php echo base_url('tenant/cetak'); ?>" method="post" target="_self">
