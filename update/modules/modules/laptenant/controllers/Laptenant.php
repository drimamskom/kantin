...
class Laptenant extends CI_Controller {
	...
	function __construct(){
            ...
            $this->load->model('m_laptenant');
	}
        
	public function index(){ 
            $this->load->view('header');
            $this->load->view('laptenant');    
    ...
    public function data(){            
        ...
        if(empty($kata)){
            ...
            $where1=" AND ( p.kode_trns_pemesanan LIKE '%$kata%' OR pp.kode LIKE '%$kata%' OR s.nama LIKE '%$kata%' OR dm.nama_barang LIKE '%$kata%' OR p.stat LIKE '%$kata%' ) ";
        }
        if(empty($tgl_mulai)||empty($tgl_selesai)){
            $where2="";
        }else{
            $d1 = $this->m_laptenant->datetosqldate($tgl_mulai);
            $d2 = $this->m_laptenant->datetosqldate($tgl_selesai);
        ...
        $query1 = $this->db->query("SELECT COUNT(*) AS total, SUM(total) AS jumm 
                                    ...
                                    WHERE p.kode_trns_pemesanan IS NOT NULL $where1 $where2 AND dm.no_stan='$nomor_induk' ");
        ...
        foreach ($query->result_array() as $row){
            ...
            // $data[$i] = $row;
            $row['tgl']   = $this->m_laptenant->sqldatetodate($row['tgl']);
            $row['button'] = '<button title="hapus" idnex="'.$row['id_trns_pemesanan_detail'].'" namenex="'.$row['kode_trns_pemesanan'].'" class="btn btn-danger btn-xs btnhapus" ><i class="fa fa-remove"></i> Del</button>';
            $data[$i] = $row;  
            $i++;
        }
    ...
    public function hapus(){
        ...
        // cari data masih ada         
        $query1 = $this->db->query("SELECT * FROM trns_pemesanan_detail WHERE id_trns_pemesanan_detail='$id' ");
        $jml = $query1->num_rows(); 
        if($jml!='0') { 
            // $stat = "batal";
            //$delete1 = $this->db->query("DELETE FROM trns_pemesanan WHERE kode_trns_pemesanan='$id' ");
            $delete2 = $this->db->query("DELETE FROM trns_pemesanan_detail WHERE id_trns_pemesanan_detail='$id' ");
            // $update = $this->db->query("UPDATE trns_pemesanan_detail SET stat='$stat' WHERE id_trns_pemesanan_detail='$id' ");
    ...
    public function export(){
        ...
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, 1, "Daftar Pesanan ".$nama);
        $objPHPExcel->getActiveSheet()->setCellValueByColumnAndRow($col, 2, "Tanggal :  ".$tanggal);
        ...
        //Set Title
        $objPHPExcel->getActiveSheet()->setTitle('Data Pemesanan Tenant');
        ...
        //Nama File
        header('Content-Disposition: attachment;filename="Lap_Tenant.xlsx"');
    ...
    public function dataexport(){            
        ...
        $query1 = $this->db->query("SELECT COUNT(*) AS total, SUM(total) AS jumm 
                                    ...
                                    LEFT JOIN tb_customer s ON s.kode=pp.kode
                                    LEFT JOIN tb_stok dm ON dm.kode_barang=p.kode_menu
                                    WHERE p.kode_trns_pemesanan IS NOT NULL AND pp.tgl='$tgl' AND dm.no_stan='$nomor_induk' ");
        $row1 = $query1->first_row();
        $tot = $row1->total;
        $totalz = $row1->jumm;
