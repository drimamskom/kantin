...
<script type="text/javascript">   
	$(function(){ 
            $('#table_cust').DataTable({
                ...
                "serverSide": true,
	        "ajax": {
	            "url": "<?php echo base_url('trnspembelian/data'); ?>",
                    "data": function( d ) {
	    ...
	    $(document).on("click","#btnexport",function(){
            ...
            $(document).on("click",".btnview",function(){
                ...
                resetForm();
                $.ajax({
                    url: "<?php echo base_url('trnspembelian/cari'); ?>",
                    type: "POST",
                    data : value,
                    dataType: "json",
			success: function(data){
                            ...
                            for (var i = 0; i < detail.length; i++) {
                                ...
                                var tr = '<tr class="temp-row">'+  
                                        ...
                                        '<td align="center">'+ detail[i].qty +'</td>'+ 
			...
			success: function(data){
                            var head = data.header[0];  
                            $("#inp3_faktur").val(head.kode_faktur);
                            $("#inp3_supplier").val(head.kode_supplier);
                            $("#text3_kdpembelian").html(head.kode_faktur);
                            $("#view3_txtnota").html(head.kode_faktur);
                            $("#view3_txtsupplier").html(head.nama_supplier);
                            $("#view3_txtpembayaran").html(head.pembayaran);
                            $("#view3_txttanggal").html(head.tgl);
                            $("#view3_txtjatuhtempo").html(head.jatuh_tempo);
                            $("#view3_txttempo").html(head.hari_jatuh_tempo+" Hari");
                            ...
                            for (var i = 0; i < detail.length; i++) {
                                ...
                                var tr = '<tr class="temp3-row">'+  
                                        ...
                                        '<td>Rp. '+ addPeriod(detail[i].total) +'</td>'+ 
                                        '<td>'+
                                            '<input type="text" class="form-control number-only kode" name="kode['+ detail[i].kode_barang +']" required/>'+
                                            '<input type="hidden" class="form-control number-only harga" name="harga['+ detail[i].kode_barang +']" value="'+ detail[i].harga_satuan +'" />'+
                                            '<input type="hidden" class="form-control number-only satuan" name="satuan['+ detail[i].kode_barang +']" value="'+ detail[i].satuan +'" />'+
                                        '</td>'+ 
                                '</tr>';  
                                $('#list_infois3 > tbody').append(tr);
                            }
                    
                            var hasil = addPeriod(head.subtotal);
                            $('#view3_total').html("Rp. "+addPeriod(alltot));
                            $("#modalcust3").modal('show');
            ...
            $('#formRetur').on('submit', function(e) {
                ...
                $.ajax({
                    url : "<?php echo base_url('trnspembelian/save'); ?>",
                    type: "POST",
                    data : post_data,
                    dataType: "json",
                    success: function(data){
                        ...
                        if(data.status == 'success'){
                            ...
                            $("#modalcust3").modal("hide");
                            $("#formRetur").trigger('reset'); 
                            resetForm3();
                        }else{
        ...
        function resetForm3(){       
            $("#text3_kdpembelian").html(" "); 
            $("#view3_txtnota").html(" "); 
            $("#view3_txtsupplier").html(" "); 
            $("#view3_txttanggal").html(" ");
            $('#view3_subtotal').html(" "); 
            $('.temp3-row').remove();
	}
    ...
    <div class="col-md-12">
        ...
        <div class="box box-primary">
            <div class="box-header with-border">
                <h3 class="box-title">Laporan Transaksi Pembelian Barang</h3>
            </div>
            <div class="box-body"> 
                <form id="export_form" action="<?php echo base_url('trnspembelian/export'); ?>" method="post" target="_self">
                            ...
                            <li style="padding-left:10px;">
                                <div class="input-group">
                                    ...
                                    <select class="form-control selectpicker" name="supplier" id="supplier" data-width="200px" title="Pilih...">
                                        ...
                                        <?php
                                          $query = $this->db->query("SELECT * FROM tb_supplier WHERE tempat='2' ");
...
<div id="modalcust3" class="modal">
    <div class="modal-dialog modal-lg">
        <div class="modal-content">
            ...
            <div class="modal-header">
                <button type="button" class="close" data-dismiss="modal">×</button>
                <h4 class="modal-title">Retur <small>#<span id="text2_kdpembelian"></span></small></h4>
            </div>
            <!--modal body-->
            <div class="modal-body">
                <!-- info row -->
                <div class="row">
                    <div class="col-sm-12">
                        <input type="hidden" name="nofaktur" id="inp3_faktur">
                        <input type="hidden" name="supplier" id="inp3_supplier">
                        <table width="100%">
                            <tr>
                                <td width="11%">No. Faktur</td>
                                <td width="1%">:</td>
                                <td width="30%"><span id="view3_txtnota"></span></td>
                                <td width="29%"></td>
                                <td width="11%">Tanggal</td>
                                <td width="1%">:</td>
                                <td width="17%"><span id="view3_txttanggal"></span></td>
                            </tr>
                            <tr>
                                <td>Supplier</td>
                                <td>:</td>
                                <td><span id="view3_txtsupplier"></span></td>
                                <td></td>
                                <td>Jatuh Tempo</td>
                                <td>:</td>
                                <td><span id="view3_txtjatuhtempo"></span></td>
                            </tr>
                            <tr>
                                <td>Pembayaran</td>
                                <td>:</td>
                                <td><span id="view3_txtpembayaran"></span></td>
                                <td></td>
                                <td>Tempo</td>
                                <td>:</td>
                                <td><span id="view3_txttempo"></span></td>
                            </tr>
                        </table>
                    </div><!-- /.col -->
                </div><!-- /.row -->
                ...
                <div class="row">
                    <div class="col-xs-12 table-responsive">
                        <table id="list_infois3" width="100%" class="table table-striped">
                            <thead class="label-primary">
                                <tr>
                                    <th width="5%">#</th>
                            ...
                            <tfoot>
                                <tr>
                                    <td colspan="5"><b><span class="pull-right" style="margin-right:10px;">Subtotal</span></b></td>
                                    <td><b><span id="view3_total"></span></b></td>
                                    <td></td>
                                </tr>
                            </tfoot>
                        </table>
